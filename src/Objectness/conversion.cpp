...
public:
    ...
    UMatData* allocate(int dims0, const int* sizes, int type, void* data, size_t* step, int flags, cv::UMatUsageFlags usage_flags) const
    {
        if( data != 0 )
        {
            CV_Error(Error::StsAssert, "The data should normally be NULL!");
            // probably this is safe to do in such extreme case
            return stdAllocator->allocate(dims0, sizes, type, data, step, flags, usage_flags);
        }
        PyEnsureGIL gil;
    ...
    bool allocate(UMatData* u, int accessFlags, cv::UMatUsageFlags usage_flags) const
    {
        return stdAllocator->allocate(u, accessFlags, usage_flags);
    }
